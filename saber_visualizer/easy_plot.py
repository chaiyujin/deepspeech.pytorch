...
class plot_item(dict):
    def __init__(self, item, title="", vmin=None, vmax=None, draw_fn=None, cmap=DEFAULT_CMAP, **kwargs):
        def to_numpy(t):
        ...
        super().__init__(
            item=item, title=title, vmin=vmin, vmax=vmax,
            x_length=x_length, y_length=y_length,
            draw_fn=draw_fn, cmap=cmap, **kwargs
        )
    ...
    def __draw_index_labels(self, ax, labels, ymin, ymax):
        ...
        assert len(labels) == self.x_length
...
def __allocate_figure(num_rows, num_cols, x_length, y_length, scaling, aspect):
    import matplotlib.pyplot as plt
    img_h = 2.5
    ...
    img_w *= scaling
    # alloc
    fig_h = img_h * num_rows + gap_h * (num_rows - 1) + mar_h * 2
